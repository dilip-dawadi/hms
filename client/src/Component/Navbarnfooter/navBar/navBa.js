...
import MdMenu from '@mui/icons-material/Add';
            ...
            <NavStyles>
                <div className={'nav'}>
                    ...
                    <ul className={!showNav ? 'navItems hide-item' : 'navItems'}>
                        ...
                        <li>
                            {user?.result ? (
                                <Link className='Signin' to="/profile" onClick={() => setShowNav(!showNav)}
                                    role="button"
                                    onKeyDown={() => setShowNav(!showNav)}
                                    tabIndex={0}>{user?.result?.cart.length}</Link>
                            ) : (
                    ...
                    <NavLink to="/cart" className='cart'><FaShoppingCart /> <span> {user?.result?.cart.length === 0 ? 0 : user?.result?.carts.length}</span></NavLink>
                </div>
            </NavStyles>
        </Paper>
    );
